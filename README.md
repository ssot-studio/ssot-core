...
`types.ts` 의 `SsotKind` union 과 `ssot-v1.schema.json` 의 kind enum / id prefix 가 1:1로 동기화된다.
어긋나면 normalize 가 `unknownKind`/`invalidId` 를 토해낸다.

| kind | id prefix | 의미 |
|------|-----------|------|
| `Platform` | `platform` | 제품 전체의 최상위 노드 |
| `Persona` | `persona` | 사용자/이해관계자 유형 |
| `Domain` | `domain` | 도메인(바운디드 컨텍스트) |
| `Concept` | `concept` | 도메인 개념 |
| `Capability` | `capability` | 제품 기능/역량 |
| `SystemComponent` | `component` | 시스템 구성 요소 |
| `Integration` | `integration` | 외부 연동 |
| `Invariant` | `invariant` | 불변식/제약 |
| `Decision` | `decision` | 의사결정 기록 |
| `EngineeringRule` | `rule` | 엔지니어링 규칙 |
| `Screen` | `screen` | 화면 (FE 라우트 단위) |
| `Endpoint` | `endpoint` | API 엔드포인트 |
| `Flow` | `flow` | 화면·엔드포인트를 잇는 흐름 |
